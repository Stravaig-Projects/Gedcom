...
public class RenderSourceIndexAsMarkdownHandler : RequestHandler<RenderSourceIndex>
{
    ...
    private void WriteIndex(TextWriter writer, SourceEntry[] entries)
    {
        ...
        foreach (var entry in orderedEntries)
        {
            ...
            string imagesExtension = GetImagesExtension(entry.Source);
            writer.WriteLine($"* [{entry.Source.Title.RemoveNamesOfTheLiving(entry.Source.ReferencedBy)}]({filePath}){imagesExtension}");
            var references = entry.ReferencedByIndividuals
                .OrderByStandardSort()
                .ToArray();
            foreach (var person in references)
            {
                writer.Write("  * Referenced from: ");
    ...
    private string GetImagesExtension(GedcomSourceRecord source)
    {
